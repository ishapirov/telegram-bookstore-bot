...
@Component
public class SelectUZSCommand implements Command {
    @Autowired
    CurrencySelectionController currencySelectionController;

    private final ButtonAction actionWhichTriggersCommand = ButtonAction.SELECT_UZS;

    @Override
    public Object execute(UserCallbackRequest userCallbackRequest) {
        UpdatedCurrency updatedCurrency = new UpdatedCurrency(userCallbackRequest.getUserId(),"UZS");
